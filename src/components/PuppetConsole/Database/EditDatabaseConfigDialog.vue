...
const loadEditingConfig = () => {
  ...
  const variant = connection.variant || (dialect === 'sqlite' ? 'file' : 'default')
  Object.assign(form, {
    dialect,
...
const doSaveConnection = async () => {
  await handleFormSubmit(
    async () => {
      const connection = buildDatabaseConnection(form, { omitEmptyPassword: true })
      const requestData = {
        sessionId: props.sessionId,
        connection
      }

      if (props.editingConfig && props.editingConfig.connectionId) {
...
const handleSaveConnection = async () => {
  ...
  if (!isComplete.value) {
    showWarning('请填写完整的连接信息（数据库类型、连接URL）')
    return
  }

  if (!form.password && props.editingConfig?.connectionId) {
...
.url-examples {
  ...
  border-left: 0.1875rem solid var(--el-color-primary);
}
