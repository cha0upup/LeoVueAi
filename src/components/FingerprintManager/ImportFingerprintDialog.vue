  ...
  >
    ...
    >
      <el-upload
        ...
        accept=".json,.zip"
        drag
        class="fingerprint-uploader"
      >
        <el-icon class="el-icon--upload">
          <Icon :icon="iconMap.uploadFilled" />
        </el-icon>
        <div class="el-upload__text">
          将 .json 或 .zip 拖到此处，或<em>点击选择</em>
        </div>
        <template #tip>
          <div class="el-upload__tip">
            支持单条 .json、对象数组 .json 或 .zip（内含若干 .json）
          </div>
        </template>
      </el-upload>
      ...
      <div class="form-section">
        ...
        <div class="form-tip">
          fingerprintId 由 name + version 派生，同名同版本视为冲突。
          重命名会在 version 后追加时间戳。
        </div>
      </div>
    </div>
...
import { importFingerprintsApi } from '@/services/api.js'
...
const submit = async () => {
  ...
  try {
    ...
    const res = await importFingerprintsApi(fd)
...
.fingerprint-uploader :deep(.el-upload-dragger) {
  padding: 32px 24px;
}
